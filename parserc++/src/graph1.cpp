...
Graph1::Graph1(Domain *domain, Problem *problem):m_domainptr(domain), m_problemptr(problem){}
...
vector<DurativeAction * > * Graph1::instanceActions(){
...
vector<DurativeAction *>* Graph1::instanciation(vector<vector<Object * > > * objects,DurativeAction * action){
...
bool Graph1::actionUsable(DurativeAction *action, vector< Fluent > * fluents){
...
bool Graph1::compareVV(vector<Member * >* v1 ,vector<Member * >*v2){
	if (v1->size () != v2->size()){
		return false;
	}
...
//true if f is in v
bool Graph1::compareFVF(vector<Fluent  >* v,Fluent * f){
...
DurativeAction * Graph1::make_actionGoal(){
...
DurativeAction * Graph1::make_actionInit(){
...
DurativeAction * Graph1::findAction(Vertex * v,DurativeAction * initAction,Fluent * f){
...
bool Graph1::compareFVF2(vector<Fluent  *> v,Fluent * f){
	for(unsigned i =0 ; i<v.size();++i){
		if (f->getPredicate()->getName() == v.at(i)->getPredicate()->getName()){
			if (compareVV(f->getMembersList(),v.at(i)->getMembersList() )){
				return true;
...
Vertex* Graph1::getVertex(){
	return m_vertex;
}

DurativeAction* Graph1::getInits(){
	return m_inits;
}

DurativeAction* Graph1::getGoals(){
	return m_goals;
}
